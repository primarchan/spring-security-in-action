...
- 다단계 인증(MFA) 은 사용자가 리소스에 접근할 때 여러 번 다른 방법으로 인증하도록 요청하는 인증 전략이다. 이 예에서 사용자는 사용자 이름과 암호로 인증한 후 SMS 메세지를 통해 바든 OTP 를 제시하여 특정 전화번호를 가지고 있음을 입증해야했다. 이 방식으로 자격 증명 도난으로부터 사용자의 리소스를 더 잘 보호할 수 있다.
- 한 가지 문제에 대한 좋은 해결책이 두 개 이상인 경우가 많다. 항상 가능한 모든 해결책을 고려하고 시간이 허용된다면 모든 옵션의 개념 증명을 구현해 시니리오에 가장 적합한 옵션을 찾는 것이 좋다.

## 12장. OAuth 2 가 작동하는 방법
- 